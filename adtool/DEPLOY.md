...
上面几步跑通只是「服务器上能跑起来」。真正对外宣布之前,按这个顺序切,
关键是**别让两套系统同时被人写**——切换那一刻数据就分叉了,事后没法合。

**切换前**

1. 服务器上自己完整走一遍:登录、翻词库、加一条再删掉、导一次表,确认功能正常
2. 挑一个同事先试(不同电脑、不同浏览器),排除「只有你这台能开」
3. 备份 cron 装好并手动跑一次,确认能生成 tar.gz(见第三节)

**切换当天**

4. **先停掉你电脑上那套**。这是最关键的一步 —— 停之前发一条通知,让大家先别录入
5. 停掉之后,把你本机 `server/data/` 里的三个数据库文件(`.db` / `-wal` / `-shm`)
   scp 到服务器覆盖 `data-prod/`,确保切过去的是最新数据,不是仓库里那份旧快照:

   ```bash
   # 在你的 Windows 上
   scp adtool\server\data\adtool.db* kevin@192.168.53.9:~/apps/AD-TOOL/adtool/data-prod/
   ```

   ```bash
   # 服务器上重启让它读新库
   cd ~/apps/AD-TOOL/adtool && docker compose restart && docker compose logs --tail=20
   ```

6. 自己再登一次确认数据是最新的(找一条只有最新库里才有的词验证)
7. 通知全员新地址 `http://192.168.53.9:8080`,说明三件事:
   - 要重新登录一次(换地址 + session 密钥变了),**账号密码不变**
   - 旧地址作废,书签换掉
   - 以后你电脑关不关机都不影响使用

**切换后一周**

8. 你电脑上那套**别删,也别再启动**。留着当应急回退,一周后确认无事再清理
9. 每天扫一眼 `docker compose ps` 和备份目录,确认容器没重启、tar.gz 在正常生成
