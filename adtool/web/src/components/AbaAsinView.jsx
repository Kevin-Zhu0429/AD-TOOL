...
const defaults = { model: '', view: 'queries', month: '', asin: '', skuId: '', q: '', wordType: 'all', merge: '1', sort: 'market_impressions', direction: 'desc', page: 1, pageSize: 100 };
...
export default function AbaAsinView({ market, userId }) {
  ...
  function saved(result) {
    ...
    change({ year: '', month: '', model: '', asin: '', skuId: '', weeks: weeks.join(',') });
    setRevision((n) => n + 1);
  }
  return <div className="aba-asin-view">
    ...
    <section className="aba-filters" aria-label="ASIN 报告筛选">
      <div className="aba-toolbar aba-asin-selectors">
        <label>年份<select className="inp" aria-label="年份" value={filters.year ?? data?.year ?? ''} onChange={(e) => change({ year: e.target.value, month: '', model: '', asin: '', skuId: '', weeks: undefined })}><option value="">全部年份</option>{data?.years.map((y) => <option key={y} value={y}>{y} 年</option>)}</select></label>
        <label>月份<select className="inp" aria-label="月份" value={filters.month} onChange={(e) => change({ month: e.target.value, model: '', asin: '', skuId: '', weeks: undefined })}><option value="">全部月份</option>{data?.months.map((m) => <option key={m} value={m}>{Number(m)} 月</option>)}</select></label>
        <label>墨盒型号<select className="inp" aria-label="墨盒型号" value={filters.model} onChange={(e) => change({ model: e.target.value, asin: '', skuId: '', weeks: undefined })}><option value="">不合并套组</option>{data?.modelOptions.map((m) => <option key={m.key} value={m.key}>{m.label}（{m.asins.length} 个 ASIN）</option>)}</select></label>
      ...
      <p className="hint aba-model-note">年份、月份按报告结束日归属，周数取自原始 CSV 的 C1 或合并表的 AJ 列。SKU 仅关联当前账号在 {market} 站的记录；选择墨盒型号后合并该型号的颜色套组；同一 ASIN 对应多个 SKU 只计一次。未关联型号的 ASIN 可单独查看。</p>
      <div className="aba-toolbar">
        ...
        <label className="aba-model-toggle"><input type="checkbox" disabled={filters.view === 'printers'} checked={filters.view === 'printers' || filters.merge === '1'} onChange={(e) => change({ merge: e.target.checked ? '1' : '0' })} />多周合并相同搜索词</label>
      </div>
