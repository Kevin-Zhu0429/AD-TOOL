...
const storageKey = (userId, market) => `aba-asin-filters:${isPet ? 'pet:' : ''}${userId}:${market}`;
...
function restore(userId, market) {
  try {
    ...
    return { ...defaults, ...saved, ...(isPet ? { model: '', view: 'queries', wordType: 'all' } : {}) };
  }
...
export default function AbaAsinView({ market, userId }) {
  ...
  const skuLabel = (s) => (isPet ? [s.sku, s.brand, s.style, s.size, s.color, s.fabric] : [s.sku, s.brand, s.model ? `${s.model} 系列` : '', s.setGroup]).filter(Boolean).join(' · ');
  ...
  async function exportGroups() {
    ...
    try {
      const exported = await api.abaAsin({ ...filters, marketplace: market, view: isPet ? 'queries' : 'printers', export: '1', page: 1 }, controller.signal);
      if (isPet) {
        const columns = [{ key: 'asin', label: 'ASIN' }, { key: 'linked_skus', label: '关联 SKU（非分摊）' }, ...ASIN_COLUMNS.slice(1), { key: 'period', label: '报告周' }];
        const rows = exported.items.map((row) => ({ ...row,
          linked_skus: exported.skuItems.filter((s) => s.asin === row.asin).map((s) => s.sku).join(' / '),
          period: row.periods?.map((p) => p.week_end).join(' / ') || row.week_end }));
        downloadReportRows(columns, rows, '美国站 ASIN 搜索词');
        return;
      }
  ...
  return <div className="aba-asin-view">
    ...
    <section className="aba-filters" aria-label="ASIN 报告筛选">
      <div className="aba-toolbar aba-asin-selectors">
        ...
        {!isPet && <label>墨盒型号<select className="inp" aria-label="墨盒型号" value={filters.model} onChange={(e) => change({ model: e.target.value, brand: '', asin: '', skuId: '', weeks: undefined })}><option value="">不合并套组</option>{data?.modelOptions.map((m) => <option key={m.key} value={m.key}>{m.label}（{m.asins.length} 个 ASIN）</option>)}</select></label>}
      ...
      {isPet && <div className="pet-filters">{[['style', '款式'], ['size', '尺码'], ['color', '颜色']].map(([key, label]) => <label key={key}>{label}<select className="inp" aria-label={label} value={filters[key] || ''} onChange={(e) => change({ [key]: e.target.value, asin: '', skuId: '', weeks: undefined })}><option value="">全部{label}</option>{(data?.attributes?.[key] ?? []).map((v) => <option key={v}>{v}</option>)}</select></label>)}</div>}
      {isPet ? <p className="hint">属性来自店铺共享的美国站 SKU。筛选属性用于选择关联 ASIN；多个 SKU 对应同一 ASIN 时，报告数据只计一次，不分摊为 SKU 的表现。</p> : <p className="hint aba-model-note">年份、月份按报告结束日归属，周数取自原始 CSV 的 C1 或合并表的 AJ 列。SKU 仅关联当前账号在 {market} 站的记录；按墨盒型号 → 品牌 → ASIN → SKU 逐级筛选，型号列出各品牌已关联的型号；选择型号后合并筛选范围内的颜色套组；同一 ASIN 对应多个 SKU 只计一次。未关联型号的 ASIN 可单独查看。</p>}
      {!!data?.unlinkedAsins?.length && <details className="aba-prices"><summary>{data.unlinkedAsins.length} 个 ASIN {isPet ? '未关联 SKU' : '未关联墨盒型号'}，点击查看</summary><p className="hint">{isPet ? '请在店铺共享 SKU 库补充 ASIN，保存后重新加载报告；未关联时仍可单独查看 ASIN。' : '请在当前账号、当前站点的 SKU 库补充 ASIN、品牌和型号，保存后重新加载报告。未关联时仍可从 ASIN 筛选查看。'}</p>{data.unlinkedAsins.map((asin) => <span key={asin}>{asin}</span>)}</details>}
      <div className="aba-toolbar">
        <div className="aba-search"><label htmlFor="aba-asin-search">{isPet ? '搜索查询' : '搜索查询 / 墨盒型号'}</label><div className="aba-search-control"><input id="aba-asin-search" ref={searchRef} className="inp" value={search} placeholder={isPet ? "如 dog raincoat，按搜索词原文查找" : "如 305，包含对应机型词"} onChange={(e) => setSearch(e.target.value)} onCompositionStart={() => setComposing(true)} onCompositionEnd={() => setComposing(false)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.nativeEvent.isComposing) change({ q: search }); }} />{search && <button className="btn ghost" aria-label="清除搜索" onClick={clearSearch}>清除</button>}</div></div>
        {!isPet && <label>词类型<select className="inp" aria-label="词类型" value={filters.wordType} onChange={(e) => change({ wordType: e.target.value })}><option value="all">全部搜索词</option><option value="printer">仅机型词</option><option value="cartridge">仅墨盒 KW 词</option></select></label>}
      ...
      {!isPet && data && !data.hasModelLibrary && <p className="hint aba-model-note">当前区域没有 D 类机型库，搜索词暂归为墨盒 KW 词；补充机型库后会重新识别。</p>}
    </section>
    ...
    <div className="aba-results" aria-busy={loading}>
      {!loading && !error && data ? <section className="aba-table-panel">
        <div className="aba-table-heading"><div><h2>{data.view === 'printers' ? 'ASIN 机型分类汇总' : 'ASIN 搜索查询明细'}</h2><p className="hint">{data.aggregation === 'average' ? '按搜索词实际出现周数计算周平均，可展开分类核对明细。' : data.view === 'printers' ? '按 ASIN 和机型分类汇总全部所选周，展开查看搜索词；未识别到机型的词归为墨盒 KW 词。' : data.merged ? '同 ASIN、同搜索词跨周合并，百分比按合计次数重算。' : '每行一个 ASIN 的一条搜索词周记录。'} {data.seriesMerged ? '当前型号跨套组合并：同词同周市场数据只计一次，ASIN 指标相加；市场数值冲突时显示待核对。' : '不同 ASIN 的市场数据有重叠，分别展示。'}</p></div>{(isPet || data.view === 'printers') && <div className="aba-table-actions">{!isPet && <label className="aba-model-toggle"><input type="checkbox" checked={hideCodes} onChange={(event) => setHideCodes(event.target.checked)} />隐藏 ASIN / SKU</label>}<button className="btn" disabled={exporting || !data.items.length} aria-busy={exporting} onClick={exportGroups}>{exporting ? '正在导出…' : '导出 Excel'}</button></div>}</div>
        {exportError && <p className="aba-export-feedback aba-error-text" role="alert">{exportError}</p>}
        {!isPet && data.aggregation === 'average' && <p className="hint aba-model-note">周平均：每个搜索词的数量按实际出现周数平均，未出现的周不计入；同词跨套组先合并再平均。机型分类汇总为各词周平均之和，百分比按当前显示的数量重算。</p>}
