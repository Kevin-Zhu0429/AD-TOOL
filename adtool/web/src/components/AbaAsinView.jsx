...
export default function AbaAsinView({ market, userId }) {
  ...
  return <div className="aba-asin-view">
    ...
    <section className="aba-filters" aria-label="ASIN 报告筛选">
      ...
      {isPet ? <p className="hint">属性来自当前账号的美国站 SKU。筛选属性用于选择关联 ASIN；多个 SKU 对应同一 ASIN 时，报告数据只计一次，不分摊为 SKU 的表现。</p> : <p className="hint aba-model-note">年份、月份按报告结束日归属，周数取自原始 CSV 的 C1 或合并表的 AJ 列。SKU 仅关联当前账号在 {market} 站的记录；按墨盒型号 → 品牌 → ASIN → SKU 逐级筛选，型号列出各品牌已关联的型号；选择型号后合并筛选范围内的颜色套组；同一 ASIN 对应多个 SKU 只计一次。未关联型号的 ASIN 可单独查看。</p>}
      {!!data?.unlinkedAsins?.length && <details className="aba-prices"><summary>{data.unlinkedAsins.length} 个 ASIN {isPet ? '未关联 SKU' : '未关联墨盒型号'}，点击查看</summary><p className="hint">{isPet ? '请在自己的 SKU 库补充 ASIN，保存后重新加载报告；未关联时仍可单独查看 ASIN。' : '请在当前账号、当前站点的 SKU 库补充 ASIN、品牌和型号，保存后重新加载报告。未关联时仍可从 ASIN 筛选查看。'}</p>{data.unlinkedAsins.map((asin) => <span key={asin}>{asin}</span>)}</details>}
