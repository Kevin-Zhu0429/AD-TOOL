      ...
      <div className="aba-results" aria-busy={loading}>
        {!error && !loading && data && <>
          ...
          <section className="aba-table-panel">
            <div className="aba-table-heading"><div><h2>{data.view === 'printers' ? '机型分类汇总' : '搜索查询明细'}</h2><p className="hint">{data.view === 'printers' ? '每个分类统计全部所选周，展开查看其搜索词；仅展示有报告数据的分类。' : data.merged ? '相同搜索词跨周合并，市场与品牌的数量分别相加，百分比按总数重算。' : '每行一个搜索词的一周数据。'}</p></div><button className="btn" aria-pressed={focused} onClick={() => setFocused((v) => !v)}>{focused ? '退出专注明细' : '专注明细'}</button></div>
