...
export default function AbaReportUpload({ market, kind = 'brand', onSaved }) {
  ...
  async function choose(fileList) {
    ...
    setError(''); setNotice('');
    if (files.length > (isAsin ? 200 : 10)) return setError(`每次最多上传 ${isAsin ? 200 : 10} 份报告文件。`);
    ...
    for (const file of files) {
      try {
        ...
        if (isAsin && /\.xlsx$/i.test(file.name)) {
          ...
          next.push({ name: file.name, reports });
          continue;
        }
        let text;
  ...
  async function save() {
    ...
    try {
      let result;
      if (isAsin) {
        const batches = asinUploadBatches(pending, market);
        const total = batches.reduce((sum, batch) => sum + batch.length, 0);
        for (const [index, batch] of batches.entries()) {
          if (!mounted.current) return;
          setProgress(`正在保存第 ${index + 1}/${batches.length} 批，已完成 ${savedReports.length}/${total} 份周报…`);
          const part = await api.importAbaAsin(market, batch);
          savedReports.push(...part.reports);
        }
        result = { reports: savedReports };
      } else result = await api.importAba(market, pending.map(({ name, text }) => ({ name, text })));
      if (!mounted.current) return;
      const labels = { added: '已保存', updated: '已更新', unchanged: '已存在，无需重复保存' };
      setNotice(result.reports.length > 20 ? `已完成 ${number.format(result.reports.length)} 份周报，共 ${number.format(result.reports.reduce((sum, r) => sum + r.count, 0))} 条记录。` : result.reports.map((r) => `${r[kind]} ${r.week_end}：${labels[r.status]} ${number.format(r.count)} 条`).join('；'));
      setPending([]); onSaved(result);
    } catch (err) {
      if (mounted.current) {
        setError(`${err.message}。${savedReports.length ? `此前已保存 ${savedReports.length} 份周报。` : ''}文件仍保留，可重试；相同报告重试不会重复累计。`);
        if (savedReports.length) onSaved({ reports: savedReports });
      }
    } finally { busy.current = false; if (mounted.current) { setUploading(false); setProgress(''); } }
  }
  return <section className={`aba-upload${dragging ? ' dragging' : ''}`} aria-label={`上传${label}报告`}
    ...
    <div className="aba-upload-top"><div><strong>上传{label}视图周报</strong>
      <p className="hint">{isAsin ? '支持原始 CSV 周报和合并 XLSX 表，每次最多 200 个文件，合并表最多 5,000 份周报；自动分批保存。' : '拖入原始 CSV，或选择文件，每次最多 10 份。'}单份 10 MB，合计 30 MB。</p>
    ...
    <div className="aba-feedback" aria-live="polite">{error ? <span className="aba-error-text" role="alert">{error}</span> : progress || notice || (reading ? '正在读取并校验文件…' : '')}</div>
  </section>;
}
